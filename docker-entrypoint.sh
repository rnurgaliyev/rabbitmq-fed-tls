...
: "${RABBITMQ_SSL_CACERTS:=${RABBITMQ_SSL_CA_CERTS:-}}"
...
: "${RABBITMQ_MANAGEMENT_SSL_CACERTS:=$RABBITMQ_SSL_CACERTS}"
...
if [ "$haveSslConfig" ]; then
	missing=()
	for sslConf in cacertfile cacerts certfile keyfile; do
...
rabbit_env_config() {
	...
	for conf; do
		...
		case "$conf" in
			...
			cacertfile|cacerts|certfile|keyfile)
				[ "$val" ] || continue
